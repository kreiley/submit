               ...
               thread_func *function, void *aux) 
{
  ...
  struct status * child = give_birth(t->tid);
  t->process_status = child;

/*DIRECTORY STUFF------------------------------------*/
  if(thread_current()->current_working_dir !=NULL){
       t->current_working_dir = dir_reopen(thread_current()->current_working_dir);
  }
  else{
...
void remove_locks(void){
	...
	struct list_elem * i = list_begin (&t->lock_list);
	while(i != list_end(&t->lock_list)){
		next = list_next(i);
